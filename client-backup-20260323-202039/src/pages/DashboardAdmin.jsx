...
const DashboardAdmin = () => {
  ...
  const handleVerifyAdminPassword = async (e) => {
    ...
    try {
      ...
      await apiService.verifyPassword(adminPassword);
  ...
  const normalizedSearch = programSearchQuery.trim().toLowerCase();
  ...
  const filteredDepartments = adminDepartments.filter((department) => {
    if (!normalizedSearch) {
      return true;
    }

    const code = (department.code || '').toString().toLowerCase();
    const name = (department.name || '').toString().toLowerCase();
                  ...
                  {isLoadingDepartments ? (
                    ...
                    <p>
                      {programSearchQuery.trim()
                        ? `No programs found for "${programSearchQuery.trim()}".`
