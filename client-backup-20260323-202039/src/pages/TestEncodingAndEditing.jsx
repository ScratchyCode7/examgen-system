...
import { 
    ...
    Bold, Italic, Underline, Link, List, ListOrdered, Sigma, Edit, 
    Heading1, Heading2,
} from 'lucide-react';
...
const MathSymbolPicker = ({ position, onSelect, onClose }) => {
    ...
    useEffect(() => {
        ...
        try {
            const rendered = katex.renderToString(formulaValue, {
                throwOnError: false,
            });
            ...
            setPreviewHtml('');
            setRenderError('Unable to render formula. Check your LaTeX syntax.');
        }
    }, [formulaValue]);

    const handleSymbolClick = (symbol) => {
        if (activeView === 'builder') {
            insertSymbolIntoFormula(symbol);
        } else {
            onSelect(symbol);
        }
    };

    const insertSymbolIntoFormula = (symbol) => {
        const textarea = textareaRef.current;
        if (!textarea) {
            setFormulaValue((prev) => `${prev}${symbol}`);
            return;
        }
        const { selectionStart = 0, selectionEnd = 0 } = textarea;
        const nextValue = `${formulaValue.slice(0, selectionStart)}${symbol}${formulaValue.slice(selectionEnd)}`;
    ...
    const handleInsertFormula = () => {
        if (!previewHtml) {
            setRenderError('Enter a LaTeX expression (e.g., \\frac{a}{b}) to insert.');
            return;
        }
        const encoded = encodeURIComponent(formulaValue.trim());
        const htmlPayload = `<span class="math-formula" data-formula="${encoded}">${previewHtml}</span>`;
        onSelect(htmlPayload, { asHtml: true });
    };
    ...
    return (
        ...
        >
            <div className="picker-header">
                <div>
                    <div className="picker-title">Formula Builder</div>
                    <div className="picker-subtitle">Type LaTeX or tap symbols</div>
                </div>
                <button onClick={onClose} className="close-btn">×</button>
            </div>
            <div className="picker-tabs">
                <button 
                    className={`picker-tab ${activeView === 'builder' ? 'active' : ''}`}
                ...
                <button 
                    className={`picker-tab ${activeView === 'symbols' ? 'active' : ''}`}
                    onMouseDown={(e) => e.preventDefault()}
            ...
            {activeView === 'builder' ? (
                <div className="formula-builder-panel">
                    <textarea
                        ref={textareaRef}
                    ...
                    />
                    <div className={`formula-preview ${previewHtml ? '' : 'empty'}`}>
                        {previewHtml ? (
                            <span dangerouslySetInnerHTML={{ __html: previewHtml }} />
                        ) : (
                            <span className="formula-placeholder">Preview will appear here</span>
                        )}
                    </div>
                    {renderError && <p className="formula-error">{renderError}</p>}
                    <div className="mini-symbol-grid">
                        {MATH_SYMBOLS.slice(0, 12).map((symbol) => (
                            <button 
                                key={`mini-${symbol}`}
                                onMouseDown={(e) => { e.preventDefault(); handleSymbolClick(symbol); }}
                                className="symbol-btn compact"
                                title={`Insert ${symbol}`}
                            >
                                {symbol}
                            </button>
            ...
            <div className="picker-footer">
                <span className="picker-hint">Supports KaTeX syntax</span>
                <button onMouseDown={(e) => e.preventDefault()} onClick={handleInsertFormula} className="formula-insert-btn">
                    Insert Formula
                </button>
            </div>
        </div>
...
const RichTextToolbar = ({ onFormat, onSaveRange, activeCommands, onImageEdit }) => {
    ...
    return (
        ...
        <div className="rich-text-toolbar" onMouseDown={(e) => e.preventDefault()}> 
            ...
            <button title="Formula Builder" 
                onMouseDown={(e) => handleToolbarClick(e, 'openPicker', 'math', e.currentTarget)}>
                <Sigma size={18} />
            </button>
            <button title="Question Image Upload" onMouseDown={(e) => handleToolbarClick(e, 'image', 'editImage')}><Edit size={18} /></button>
        </div>
    );
};
// -----------------
...
const TestEncodingAndEditing = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            if (isMathPickerOpen && !event.target.closest('.math-symbol-picker') && !event.target.closest('.rich-text-toolbar button[title="Formula Builder"]')) {
    ...
    const insertSymbol = (symbol, options = {}) => {
        const { asHtml = false, keepOpen = false } = options;
        const currentRef = activeEditableRef.current;
        if (!currentRef) {
            showToast({ message: 'Click inside a content field first.', type: 'info' });
            return;
        ...
        setTimeout(() => {
            ...
            if (asHtml) {
                const fragment = rangeToUse.createContextualFragment(symbol);
