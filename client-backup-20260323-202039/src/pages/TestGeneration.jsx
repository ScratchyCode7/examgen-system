...
const SUBJECT_QUESTION_PAGE_SIZE = 200;
const MAX_SUBJECT_QUESTION_PAGES = 10;
...
const TestGeneration = () => {
  ...
  const [examSubmittedForPrint, setExamSubmittedForPrint] = useState(false);
  ...
  const topicQuestionRequestRef = useRef(new Map());
  const subjectQuestionPageRequestRef = useRef(new Map());
  ...
  const ensureSubjectQuestionPools = React.useCallback(async (subjectId, topicList, options = {}) => {
    ...
    try {
      const response = await apiService.getQuestions({
        ...
        pageNumber: 1,
        pageSize: SUBJECT_QUESTION_PAGE_SIZE
      });
  ...
  const loadTopicQuestionsOnDemand = React.useCallback(async (topicId) => {
    if (!topicId) return [];
    const topicKey = String(topicId);
    const cached = questionsByTopic?.[topicId];
    if (Array.isArray(cached) && cached.length > 0) {
      return cached;
    }

    const inflight = topicQuestionRequestRef.current.get(topicKey);
    if (inflight) {
      return inflight;
    }
    ...
    const request = (async () => {
      ...
      setQuestionsByTopic((prev) => {
        if (prev?.[topicId] === list) return prev;
        return {
          ...prev,
          [topicId]: list
        };
      ...
      console.log(`[perf] topic ${topicId} question load: ${Math.round(performance.now() - startedAt)}ms (${list.length} items)`);
      return list;
    })();

    topicQuestionRequestRef.current.set(topicKey, request);
    try {
      return await request;
    } finally {
      topicQuestionRequestRef.current.delete(topicKey);
    }
  }, [normalizeApiArray, questionsByTopic]);

  const loadSubjectQuestionsPaged = React.useCallback(async (subjectId) => {
    if (!subjectId) return [];
    const subjectKey = String(subjectId);
    const cached = subjectQuestionCacheRef.current.get(`paged:${subjectKey}`);
    if (Array.isArray(cached) && cached.length > 0) {
      return cached;
    }

    const inflight = subjectQuestionPageRequestRef.current.get(subjectKey);
    if (inflight) {
      return inflight;
    }
    ...
    const request = (async () => {
      ...
      for (let pageNumber = 1; pageNumber <= MAX_SUBJECT_QUESTION_PAGES; pageNumber += 1) {
        ...
        console.log(
          `[perf] subject ${subjectId} page ${pageNumber} loaded in ${Math.round(performance.now() - startedPage)}ms (${items.length} items)`
        );

        if (items.length < SUBJECT_QUESTION_PAGE_SIZE) {
          break;
        }
      ...
      console.log(
        `[perf] subject ${subjectId} progressive load complete in ${Math.round(performance.now() - startedTotal)}ms (${collected.length} items)`
      );

      subjectQuestionCacheRef.current.set(`paged:${subjectKey}`, collected);
      return collected;
    })();

    subjectQuestionPageRequestRef.current.set(subjectKey, request);
    try {
      return await request;
    } finally {
      subjectQuestionPageRequestRef.current.delete(subjectKey);
    }
  }, [normalizeApiArray]);
  ...
  useEffect(() => {
    if (!selectedSubject || !Array.isArray(topicRows) || topicRows.length === 0) {
      return;
    }
    ...
    const prefetchSelectedTopicPools = async () => {
      for (const topicId of selectedTopicIds) {
        if (cancelled) return;
        try {
          await loadTopicQuestionsOnDemand(topicId);
        } catch (prefetchErr) {
          console.error(`Failed to prefetch topic ${topicId} questions:`, prefetchErr);
        }
        await new Promise((resolve) => setTimeout(resolve, 0));
      }
    };

    void prefetchSelectedTopicPools();

    return () => {
      cancelled = true;
    };
  }, [loadTopicQuestionsOnDemand, selectedSubject, topicRows]);
...
// Handle delete row - show confirmation first
  ...
  const calculateSpecification = React.useCallback(() => {
    ...
    const getQuestionsForTopicAndLevel = (topicId, level) => {
      ...
      const result = topicQuestions.filter(q => {
        ...
        return bloomLevelsToMatch.includes(q.bloomLevel);
      });
  ...
  }, [topicRows, specOverrides, totalExamItems, questionsByTopic, calculateSpecification]);
  ...
  const buildDraftExamData = React.useCallback(() => {
    const questions = Array.isArray(sampleExam?.questions)
      ? sampleExam.questions
          .map((question, index) => {
            const questionId = Number(question?.id ?? question?.questionId ?? question?.QuestionId);
            if (!Number.isInteger(questionId)) return null;

            const options = (question?.options || question?.Options || question?.choices || [])
              .map((option, optIdx) => {
                const optionId = Number(option?.optionId ?? option?.OptionId ?? option?.id ?? option?.Id);
                const content = option?.content ?? option?.Content ?? option?.optionText ?? option?.text ?? '';
                return {
                  optionId: Number.isInteger(optionId) ? optionId : 0,
                  displayOrder: Number(option?.displayOrder ?? option?.DisplayOrder ?? optIdx),
                  content,
            ...
            const image = question?.image || question?.Image;

            return {
              questionId,
              displayOrder: Number(question?.displayOrder ?? question?.DisplayOrder ?? index),
              content: question?.content ?? question?.Content ?? '',
              bloomLevel: Number(question?.bloomLevel ?? question?.BloomLevel ?? 0) || null,
              points: Number(question?.points ?? question?.Points ?? 1) || 1,
              image: image
                ? {
                    imagePath: image?.imagePath ?? image?.ImagePath ?? null,
                    widthPercentage: Number(image?.widthPercentage ?? image?.WidthPercentage ?? 50) || 50,
                    alignment: image?.alignment ?? image?.Alignment ?? 'Center'
                  }
                : null,
              options
            };
    ...
    if (!departmentId || !courseId || !subjectId || questions.length === 0) {
      return null;
    }
    ...
    return {
      ...
      courseId,
      subjectId,
      title: `${selectedSubjectDetails?.code || 'Exam'} ${examType} Draft`.trim(),
  ...
  const handleGenerateSample = async () => {
    ...
    try {
      ...
      setExamSubmittedForPrint(false);
      ...
      const topicPools = await Promise.all(selectedTopicIds.map(async (topicId) => {
        try {
          const list = await loadTopicQuestionsOnDemand(topicId);
      ...
      if (missingAfterTopicFallback > 0) {
        ...
        // Final fallback from subject-level pool regardless of bloom
        if (finalQuestions.length < total) {
          const remainingIds = new Set(finalQuestions.map(q => String(q.id)));
          const anyPool = subjectPools.any.filter(q => !remainingIds.has(String(q.id)));
          const need = total - finalQuestions.length;
          const sampled = sampleRandom(anyPool, need);
          finalQuestions.push(...sampled);
          if (sampled.length > 0) {
            warnings.push(`Filled ${sampled.length} remaining items from subject pool (any level)`);
          }
        }
      }
      ...
      setLastSavedSignature('');
      console.log(`[perf] sample generation completed in ${Math.round(performance.now() - generationStart)}ms for ${total} items`);
    } catch (err) {
  ...
  const handleSubmitPrintRequest = async (notes, copies) => {
    try {
      const hasUnsavedExamChanges = !activeExamMeta?.id || canSaveExam;

      if (hasUnsavedExamChanges) {
        const examData = buildDraftExamData();
        if (!examData) {
          setError('Cannot submit print request yet. Please select a department, program, subject, and generate questions.');
          return;
        }

        await apiService.submitPrintRequest({
          testId: activeExamMeta?.id || null,
          notes,
      ...
      setPrintRequestCopies(1);
      setError('');
      showToast({ message: 'Print request sent. The exam has been submitted for printing.', type: 'success' });
      setExamSubmittedForPrint(true);
      handleClearGeneration();
      ...
      if (!user?.isAdmin) {
        try {
          ...
          console.error('Failed to refresh print requests after submission:', refreshErr);
        }
      }
    } catch (err) {
  ...
  const hasTotalExamItems = Number(totalExamItems) > 0;
  const topicSelectionDisabled = !hasTotalExamItems || isLoadingTopics || topics.length === 0;

  return (
    <div className={`dashboard test-generation-page ${isDarkMode ? 'dark' : ''}`}>
      ...
      <div className="main-container">
        ...
        <div className="main-card">
          ...
          {selectedSubject && !hasTotalExamItems && (
            <div className="warning-message" style={{ marginTop: '0.75rem' }}>
              <span className="warning-icon">⚠️</span>
              <span>Please enter the total number of exam items before assigning topics so the generator knows how many questions to distribute.</span>
            </div>
          )}
          ...
          {selectedSubject && questionBankSummary && questionBankSummary.totalQuestions > 0 && (
            <div className="data-summary-block separate-blooms-view">
              ...
              <div className="summary-details-grid single-total-card">
                <div className="summary-card bloom-card color-1 total-encoded-card">
                  <h4 className="bloom-card-title">Total Encoded Questions</h4>
                  <p className="bloom-card-count">
                    <span style={{ fontWeight: 'bold' }}>{questionBankSummary.totalQuestions}</span>
                  </p>
                  <span className="total-encoded-subtext">Across all encoded topics</span>
                </div>
              </div>
            </div>
          )}
                            ...
                            <tr key={`spec-${idx}`}>
                              <td>
                                <select
                                  ...
                                  title={!hasTotalExamItems ? 'Enter total exam items first' : undefined}
                                >
          ...
          <div className="action-buttons">
            <button 
              ...
              disabled={!generatedSpec || isGeneratingSample}
            >
              <PlayCircle size={16} /> {isGeneratingSample ? 'Generating...' : 'Generate Sample Exam'}
            </button>
            <button 
              className="btn btn-secondary"
                ...
                <div className="exam-questions-section">
                  {!showAnswerSheet ? (
                    sampleExam.questions.map((q, idx) => {
                      const questionText = q.content || q.Content || q.question || '';
                      const resolvedQuestionHtml = typeof questionText === 'string' && questionText.trim().length > 0
                        ? questionText
                        : String(questionText || 'Question text not available');
                      ...
                      return (
                        <div key={idx} className="exam-question-item">
                          <div className="question-number-text">
                            <strong>{idx + 1}.) </strong>
                            <span
                              dangerouslySetInnerHTML={{ __html: resolvedQuestionHtml }}
                            />
                          </div>
                          {image && (
                          ...
                          <div className="question-options">
                            {options.map((option, optIdx) => {
                              ...
                              const normalizedOptionHtml = typeof optionText === 'string' ? optionText : String(optionText || '');
                              return (
                                <div key={optIdx} className="option-line">
                                  <span className="option-letter">{letter}.</span>
                                  <span
                                    className="option-text"
                                    dangerouslySetInnerHTML={{ __html: normalizedOptionHtml }}
                                  />
                                </div>
                              );
                            })}
          ...
          {examSubmittedForPrint && !generatedSpec && !sampleExam && (
            <div
              className="save-status-banner"
              style={{
                margin: '20px 0',
                padding: '12px 16px',
                borderRadius: '6px',
            ...
            >
              <strong>No active exam.</strong> Create a new one.
            </div>
          )}
