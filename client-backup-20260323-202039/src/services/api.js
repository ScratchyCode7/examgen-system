...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    const isLoginAttempt = requestUrl.includes('/api/auth/login') || requestUrl.includes('/auth/login');
...
export const apiService = {
  ...
  // Print Requests
  submitPrintRequest: async (testOrPayload, notes = null, copiesRequested = 1) => {
    const payload = (typeof testOrPayload === 'object' && testOrPayload !== null)
      ? testOrPayload
      : {
          testId: testOrPayload,
          notes,
          copiesRequested
        };
