...
const DepartmentSwitcher = ({ onDepartmentChange }) => {
      ...
      try {
        ...
        if (departmentCode && data.length > 0) {
          ...
          if (matchedDept) {
            ...
            sessionStorage.setItem('activeDepartmentId', matchedDept.id.toString());
            return;
          }
        }
        
        // Restore or set default active department
        const savedDeptId = sessionStorage.getItem('activeDepartmentId');
        if (savedDeptId && data.some(d => d.id === parseInt(savedDeptId))) {
          ...
          sessionStorage.setItem('activeDepartmentId', data[0].id.toString());
        }
      } catch (error) {
  ...
  const handleDepartmentSelect = (department) => {
    ...
    sessionStorage.setItem('activeDepartmentId', department.id.toString());
    setOpen(false);
